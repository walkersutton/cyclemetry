...
function RenderSection() {
  const { generatingImage, renderingVideo, startSecond, endSecond, config, setConfig } = useStore();
    ...
    progressIntervalRef.current = setInterval(async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Stop polling if cancelled or complete
          if (progress.status === 'cancelled' || progress.status === 'complete' || progress.status === 'error') {
    ...
    <div className="col-auto">
      <div className="card bg-light w-auto">
        <div className="card-body">
          ...
          <div className="mt-3">
            {generatingImage ? (
              <ProgressBar striped variant="warning" now={100} label={"building image"} />
            ) : null}
            {renderingVideo && renderProgress ? (
              <div>
                ...
                />
                {renderProgress.estimated_seconds_remaining && (
                  <p className="text-muted small mt-1 mb-0 text-center">
                    {(() => {
                      const totalSeconds = renderProgress.estimated_seconds_remaining;
              ...
              <ProgressBar striped variant="info" animated now={100} label={"rendering video..."} />
            ) : null}
            {error && <div className="alert alert-danger mt-2 small">{error}</div>}
            {success && <div className="alert alert-success mt-2 small">{success}</div>}
          </div>
              ...
              <div className="row g-2">
                <div className="col-4">
                  ...
                  <input
                    ...
                    value={fps}
                    onChange={(e) => updateSceneSetting('fps', parseInt(e.target.value) || 30)}
                    min={1}
                    max={60}
                  />
                </div>
                <div className="col-4">
                  ...
                  <input
                    ...
                    className="form-control form-control-sm"
                    value={width}
                    onChange={(e) => updateSceneSetting('width', parseInt(e.target.value) || 3840)}
                    min={640}
                    step={1}
                  />
                </div>
                <div className="col-4">
                  ...
                  <input
                    ...
                    className="form-control form-control-sm"
                    value={height}
                    onChange={(e) => updateSceneSetting('height', parseInt(e.target.value) || 2160)}
                    min={480}
                    step={1}
                  />
                </div>
