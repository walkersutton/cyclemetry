...
import { Card, Button, Alert, Dropdown, DropdownButton } from "react-bootstrap";
...
function InteractiveOnboarding() {
  const { gpxFilename, config, imageFilename } = useStore();
  ...
  const allStepsComplete = completedSteps.step1 && completedSteps.step2 && completedSteps.step3;

  const handleLoadDemo = () => {
    const { setGpxFilename, setDummyDurationSeconds, setStartSecond, setEndSecond, setSelectedSecond } = useStore.getState();
  ...
  const handleLoadTemplate = async (templateFilename) => {
    ...
    console.log(`🎯 Loading community template from onboarding: ${templateFilename}`);
      ...
      {allStepsComplete ? (
        <div className="text-center">
          ...
          <ul className="text-start" style={{ maxWidth: "400px", margin: "0 auto" }}>
            <li>Adjust the timeline to select which part of your ride to render</li>
        ...
        <>
            ...
            <Card.Body>
              <div className="d-flex align-items-start gap-3">
                <div style={{ minWidth: "24px" }}>
                  {completedSteps.step1 && <span className="text-success fs-5">✓</span>}
                </div>
                <div className="flex-grow-1">
                  ...
                  <p className="small text-muted mb-2">
                    Upload your GPX file or try the demo activity to get started.
                  </p>
          ...
          <Card className="mb-3">
            <Card.Body>
              <div className="d-flex align-items-start gap-3">
                <div style={{ minWidth: "24px" }}>
                  {completedSteps.step2 && <span className="text-success fs-5">✓</span>}
                </div>
                <div className="flex-grow-1">
                  ...
                  <p className="small text-muted mb-2">
                    Select a design template for your overlay. This is required to generate a preview.
                  </p>
                    ...
                    <>
                      ...
                      >
                        {communityTemplateFilenames.map((templateFilename, index) => (
                          <Dropdown.Item
                            onClick={() => handleLoadTemplate(templateFilename)}
                            key={index}
                          >
                            {templateFilename.replace('.json', '')}
                          </Dropdown.Item>
                        ))}
                      </DropdownButton>
                    </>
          ...
          <Card className="mb-3">
            <Card.Body>
              <div className="d-flex align-items-start gap-3">
                <div style={{ minWidth: "24px" }}>
                  {completedSteps.step3 && <span className="text-success fs-5">✓</span>}
                </div>
                <div className="flex-grow-1">
                  <h6 className={completedSteps.step3 ? "text-success" : ""}>
                    Preview Generated
                  </h6>
                  <p className="small text-muted mb-0">
                    Once you have an activity and template, a preview will automatically generate above.
                  </p>
